...
class TestMainSmoke:
    ...
    def test_full_run_regenerates_stakeholder_charts(
        self,
        monkeypatch,
        structured_raw_df_with_rejects,
        tmp_path,
    ):
        """Charts must be (re)generated by the training run itself so they
        cannot drift behind the metrics they visualise. Without this,
        embedding stale March charts in an April report led to text saying
        'Logistic Regression recommended' alongside charts highlighting an
        outdated CatBoost recommendation.
        """
        _patch_fast_main_dependencies(monkeypatch, structured_raw_df_with_rejects)
        ...
        # The PNGs that consumers (model card, methodology report) embed must
        # exist after a full run, regardless of how the test fixture is set up.
        for chart in (
            "stakeholder_executive_summary.png",
            "stakeholder_holdout_benchmarks.png",
            "stakeholder_top_drivers.png",
        ):
            assert (plots_dir / chart).exists(), f"Stakeholder chart not regenerated: {chart}"

    def test_chart_failure_does_not_break_run(
        self,
        monkeypatch,
        structured_raw_df_with_rejects,
        tmp_path,
    ):
        """If chart generation fails, the run must still produce its CSVs.
        The CSVs are the source of truth; charts are visual aids."""
        _patch_fast_main_dependencies(monkeypatch, structured_raw_df_with_rejects)

        # Force the chart generator to raise at import.
        import sys
        broken = type(sys)("stakeholder_charts_broken")
        broken.generate_stakeholder_charts = lambda *_args, **_kwargs: (_ for _ in ()).throw(
            RuntimeError("synthetic chart failure")
        )
        monkeypatch.setitem(sys.modules, "stakeholder_charts", broken)

        # Run must still succeed and produce the CSV artifacts.
        training_module.main(
            data_path="unused.parquet",
            optuna_trials=1,
            output_dir=str(tmp_path),
...
class TestUnderwritingOutputs:
    ...
    def test_build_applicant_score_frame_scores_post_split_decisioned_rows(self, raw_df_with_rejects):
        ...
        # PII linkage keys (authorization_id, AGE_T1) intentionally absent so the
        # output CSV cannot be tied back to a specific applicant.
        assert "authorization_id" not in applicant_scores_df.columns
        assert "AGE_T1" not in applicant_scores_df.columns
        assert "applicant_index" in applicant_scores_df.columns
        assert applicant_scores_df["applicant_index"].is_unique
