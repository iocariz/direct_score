/*===========================================================================
  ...
  Parametros:
    soc        Sociedad. Acepta etiqueta (CET / CCM) o numero (4 / 8):
                 CET <-> 4 ,  CCM <-> 8
               Se usa para el filtro society=, el nombre de libreria Oracle
               (finan4 / finan8) y el sufijo de las tablas de salida.
  ...
  Llamada simple:
    %NPE_PROCESO(soc=CET, anio=2020);
    %NPE_PROCESO(soc=8,   anio=2021);   /* equivale a CCM */

  Varias fechas/sociedades: ver el bloque %NPE_BATCH al final.
===========================================================================*/

/* Conexion Oracle: se abren ambas sociedades una sola vez, fuera de la macro,
   para no reconectar en cada llamada. La macro usara finan4 o finan8 segun
   el parametro soc. Ajusta el schema si difiere entre sociedades. */
libname finan4 oracle user="&user_dwh." pw="&pw_dwh." path='ADC'
        preserve_tab_names=yes connection=sharedread schema='DWH_FINANZAS';
libname finan8 oracle user="&user_dwh." pw="&pw_dwh." path='ADC'
        preserve_tab_names=yes connection=sharedread schema='DWH_FINANZAS';
libname wo "/risk/actual/tmp/ifrs_forward/WO_info";


%macro NPE_PROCESO(soc=, anio=, fecha_ini=, fecha_fin=,
                   out_lib=r_tmp, max_iter=50, debug=N);

    /*-- Validacion minima de parametros --------------------------------*/
    %if %length(&anio) = 0 or %length(&soc) = 0 %then %do;
        %put ERROR: NPE_PROCESO requiere los parametros soc y anio.;
        %return;
    %end;

    /*-- Mapeo sociedad: acepta etiqueta (CET/CCM) o numero (4/8) --------
         soc_num  -> filtro society= y nombre de libreria (finan4/finan8)
         soc_lbl  -> sufijo de las tablas de salida (_CET/_CCM)            */
    %local soc_num soc_lbl soc_in finlib;
    %let soc_in = %upcase(&soc);
    %if       &soc_in = CET or &soc_in = 4 %then %do;
        %let soc_num = 4; %let soc_lbl = CET;
    %end;
    %else %if &soc_in = CCM or &soc_in = 8 %then %do;
        %let soc_num = 8; %let soc_lbl = CCM;
    %end;
    %else %do;
        %put ERROR: soc no valido (&soc). Usa CET/4 o CCM/8.;
        %return;
    %end;
    %let finlib = finan&soc_num;   /* finan4 o finan8 */
    ...
    %put NOTE: ===== NPE_PROCESO soc=&soc_lbl(&soc_num) anio=&anio ventana &fecha_ini - &fecha_fin =====;

    /*-- Nombres de tablas, derivados de &anio y &soc_lbl ---------------*/
    %local in_target cet cet_desc cet_final cet_final2;
    %let in_target  = r_tmp.all_target_&anio;                      /* entrada */
    %let cet        = &out_lib..all_target_&anio._&soc_lbl;
    %let cet_desc   = &out_lib..all_target_&anio._&soc_lbl._desc;
    %let cet_final  = &out_lib..all_target_&anio._&soc_lbl._final;
    %let cet_final2 = &out_lib..all_target_&anio._&soc_lbl._final2;
    ...
    data &cet;
        ...
        set &in_target(where=(society = &soc_num));
    ...
    /*-------------------------------------------------------------------
      ...
      IMPORTANTE: en Oracle contrato_origen/contrato_fbe suelen ser
      NUMERICOS, mientras que 'contract' es caracter (put(account_id,$14.)).
      Para que todo el pipeline sea homogeneo (caracter) y casen los joins
      y los hash, convertimos ambos a $14 aqui mismo. El join se hace sobre
      el valor ya convertido.
    -------------------------------------------------------------------*/
    proc sql;
        create table work.fbe_trans as
        select distinct
               strip(put(t.contrato_origen, best14.)) as contrato_origen length=14,
               strip(put(t.contrato_fbe,    best14.)) as contrato_fbe    length=14,
               t.fch_fbe, t.tipo, t.imp_origen, t.IMP_FINAN
        from &finlib..FBE_ND_H as t
        inner join (select distinct contract from &cet) as c
            on strip(put(t.contrato_origen, best14.)) = strip(c.contract)
    ...
    /*-------------------------------------------------------------------
      ...
      OJO: la tabla wo_prectx_&soc_num._tot incluye el numero de sociedad
      (4 -> wo_prectx_4_tot, 8 -> wo_prectx_8_tot). Verifica que el nombre
      real de la tabla CCM siga este patron; si no, ajusta aqui.
    -------------------------------------------------------------------*/
    data &cet_final;
        if _n_ = 1 then do;
            declare hash hw();
            ...
            hw.defineDone();
            do until (eof1);
                set wo.wo_prectx_&soc_num._tot(where=(NATURE=392)
    ...
    %put NOTE: ===== NPE_PROCESO soc=&soc_lbl anio=&anio COMPLETADO -> &cet_final2 =====;
...
/*===========================================================================
  ...
  soc acepta etiqueta (CET/CCM) o numero (4/8). Con el patron estandar de
  fechas (dic anio -> dic anio+1) basta con pasar soc y anio.
===========================================================================*/
%NPE_PROCESO(soc=CET, anio=2020);
/*
%NPE_PROCESO(soc=CCM, anio=2020);
%NPE_PROCESO(soc=8,   anio=2021);            %* CCM por numero ;
%NPE_PROCESO(soc=CET, anio=2021);

  Ejemplo con ventana atipica (override explicito):
%NPE_PROCESO(soc=CET, anio=2023, fecha_ini='01jan2023'd, fecha_fin='30jun2024'd);
*/


/*===========================================================================
  Opcion B: batch. Itera el proceso para cada combinacion de las sociedades
  y anios indicados (producto cartesiano). Las fechas se derivan del anio.
  Ej: socs=CET CCM, anios=2020 2021  ->  4 ejecuciones independientes.
  Para ventanas atipicas usa llamadas individuales con override (Opcion A).
===========================================================================*/
%macro NPE_BATCH(socs=, anios=);
    %local i s j a;
    %let i = 1;
    %let s = %scan(&socs, &i, %str( ));
    %do %while (%length(&s) > 0);
        %let j = 1;
        %let a = %scan(&anios, &j, %str( ));
        %do %while (%length(&a) > 0);
            %NPE_PROCESO(soc=&s, anio=&a);
            %let j = %eval(&j + 1);
            %let a = %scan(&anios, &j, %str( ));
        %end;
        %let i = %eval(&i + 1);
...
/* Ejemplo: CET y CCM, para 2020 y 2021 (4 ejecuciones) */
/*
%NPE_BATCH(socs=CET CCM, anios=2020 2021);
*/
